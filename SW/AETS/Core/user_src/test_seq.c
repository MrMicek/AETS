...
static uint32_t s_relay_deadline_ms[4];
static uint32_t s_mosfet_deadline_ms[2];
...
static void test_seq_step_relays(uint32_t now_ms)
{
    for (uint8_t i = 0; i < 4U; ++i) {
        ...
        if (now_ms < s_relay_deadline_ms[i]) {
            continue;
        }
        if (s_relay_phase[i] == TEST_SEQ_PHASE_ON) {
            ...
            s_relay_deadline_ms[i] = now_ms + clamp_delay_ms(s_params.relays[i].ton_ms);
            s_relay_phase[i] = TEST_SEQ_PHASE_OFF;
            continue;
        }
        if (s_relay_phase[i] == TEST_SEQ_PHASE_OFF) {
            io_state_t next = *io_get();
            next.relays[i] = false;
            io_apply(&next);
            s_relay_deadline_ms[i] = now_ms + clamp_delay_ms(s_params.relays[i].toff_ms);
            if (s_relay_remaining[i] == 0U) {
                s_relay_phase[i] = TEST_SEQ_PHASE_DONE;
            } else {
                s_relay_phase[i] = TEST_SEQ_PHASE_ON;
...
static void test_seq_step_mosfets(uint32_t now_ms)
{
    for (uint8_t i = 0; i < 2U; ++i) {
        ...
        if (now_ms < s_mosfet_deadline_ms[i]) {
            continue;
        }
        if (s_mosfet_phase[i] == TEST_SEQ_PHASE_ON) {
            ...
            s_mosfet_deadline_ms[i] = now_ms + clamp_delay_ms(s_params.mosfets[i].ton_ms);
            s_mosfet_phase[i] = TEST_SEQ_PHASE_OFF;
            continue;
        }
        if (s_mosfet_phase[i] == TEST_SEQ_PHASE_OFF) {
            io_state_t next = *io_get();
            next.mosfet[i] = false;
            io_apply(&next);
            s_mosfet_deadline_ms[i] = now_ms + clamp_delay_ms(s_params.mosfets[i].toff_ms);
            if (s_mosfet_remaining[i] == 0U) {
                s_mosfet_phase[i] = TEST_SEQ_PHASE_DONE;
            } else {
                s_mosfet_phase[i] = TEST_SEQ_PHASE_ON;
